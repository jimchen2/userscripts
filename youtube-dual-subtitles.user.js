...
async function handleVideoNavigation() {
  ...
  async function extractSubtitleUrl() {
    ...
    const hasForeignTrack = playerData.some(({ vssId }) => /(ru|uk|de)/.test(vssId));
    if (hasForeignTrack) {
      const autoGeneratedTrack = playerData.find((track) => ["a.ru", "a.uk", "a.de"].includes(track.vssId));
      const manualTrack = playerData.find((track) => ["ru", "uk", "de"].some((code) => track.vssId.includes(code)));
