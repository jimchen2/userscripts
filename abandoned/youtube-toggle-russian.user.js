...
(function () {
  ...
  async function handleVideoNavigation() {
    console.log("FIRED");
    const { url: subtitleURL, _ } = await extractSubtitleUrl();
  ...
  async function extractSubtitleUrl() {
    ...
    const hasForeignTrack = playerData.some(({ vssId }) => /(ru|de)/.test(vssId));
    if (hasForeignTrack) {
      const autoGeneratedTrack = playerData.find((track) => ["a.ru"].includes(track.vssId));
      const manualTrack = playerData.find((track) => ["ru"].some((code) => track.vssId.includes(code)));
